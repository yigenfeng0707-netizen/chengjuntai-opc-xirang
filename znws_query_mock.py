...
SCHEMA_DDL = """CREATE TABLE bid_projects (
    id INTEGER PRIMARY KEY,
    project_name TEXT,          -- 项目名称
    industry TEXT,              -- 行业：通信工程/政企信息化/云服务/网络安全/物联网/IDC数据中心
    region TEXT,                -- 地区：杭州/宁波/温州/嘉兴/金华/台州/绍兴
    bid_date TEXT,              -- 投标日期，格式 YYYY-MM-DD
    win_amount REAL,            -- 中标金额（万元）
    status TEXT,                -- 状态：中标/未中标/进行中
    owner_user_id TEXT           -- 归属人员ID
);"""
...
def _build_nl2sql_system_prompt():
    ...
    return f"""你是专业的 NL2SQL 引擎。根据用户的自然语言问题，生成 SQLite 查询 SQL。
...
- "去年" 指 strftime('%Y', bid_date) = '{last_year}'
- 金额单位万元，列别名用英文下划线
...
def _llm_nl2sql(question: str, chart_type: str, user_id: str):
    ...
    try:
        raw = llm_client.call_llm(
            user_msg, fallback="", max_tokens=8192, temperature=0.0,
            timeout=90, reverse_order=True, thinking=True,
        ...
        # 安全校验
        sql_lower = sql.lower()
        if any(kw in sql_lower for kw in ["insert", "update", "delete", "drop", "alter", "create", "attach", "pragma"]):
            write_backend_log("llm_nl2sql_reject", {"question": question, "sql": sql}, "SQL含危险关键词，拒绝")
            return None
...
def parse_question_to_sql(question: str, chart_type: str = "table", user_id: str = ""):
    ...
    if "按行业" in q or "各行业" in q or "行业分组" in q:
        ...
        group_sql = "substr(bid_date,1,7)"
        group_col = "月份"
    elif "按状态" in q or "各状态" in q:
        group_sql = "status"
        group_col = "状态"
    elif "按年份" in q or "各年" in q:
        group_sql = "substr(bid_date,1,4)"
        group_col = "年份"
    ...
    today = datetime.date.today()
    if "本月" in q:
        where_time = f"strftime('%Y-%m',bid_date)='{today.strftime('%Y-%m')}'"
    elif "上月" in q or "上个月" in q:
        first = today.replace(day=1)
        last_month = (first - datetime.timedelta(days=1)).strftime("%Y-%m")
        where_time = f"strftime('%Y-%m',bid_date)='{last_month}'"
        ...
        where_time = f"strftime('%Y',bid_date)='{m.group(1)}'"
    elif "去年同期" in q:
...
def dataset_schema():
    ...
    schema = {
        ...
        "fields": [
            ...
            {"name": "industry", "type": "string", "comment": "行业（通信工程/政企信息化/云服务/网络安全/物联网/IDC数据中心）"},
            {"name": "region", "type": "string", "comment": "地区（杭州/宁波/温州等浙江地市）"},
            {"name": "bid_date", "type": "date", "comment": "投标日期 YYYY-MM-DD"},
            {"name": "win_amount", "type": "number", "comment": "中标金额（万元）"},
            {"name": "status", "type": "string", "comment": "状态（中标/未中标/进行中）"},
            {"name": "owner_user_id", "type": "string", "comment": "归属人员ID（行级权限）"}
        ],
        "metrics": [
...
def query_nl2sql():
    ...
    try:
        ...
        conn.close()
        return jsonify({"error": f"SQL执行失败：{e}", "sql": sql, "nl2sql_mode": nl2sql_mode}), 500
    conn.close()
    ...
    write_backend_log("intelligent_query", {"question": question, "user_id": user_id}, f"返回{len(rows)}行, SQL={sql}")
